      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          //double car_x = j[1]["x"];
			...
			// if no data of previous waypoints exists, calculate next waypoint
          	...
          	if(prev_size < 2)
          	{
				...
				//store previous path and calculate next waypoint
          	{
			...
			//Check if in front of my car, a leading car exists and find ref_v to use
          	double closestDist_s = 100000;
          	bool change_lane = false;

			double check_speed;

			bool take_head = false;
			...
			if(50>closestDist_s > 30)
			{
				ref_vel = check_speed*2.237;
			}
			else if (closestDist_s>=50)
			{
				ref_vel = 45.5;
			}
			else
			{
				ref_vel = check_speed*2.237-1.2;
			}
          	
          	//try to change lanes if too close to car in front
			// && lane > 1
			// change lanes if dist to at least one car is !zero)
			// bool not_zero(double x) {
			//	return x!=0;
			//		}
			// zero = any_of(dist.begin(),dist.end(),not_zero());



			bool lane_change_possible;
